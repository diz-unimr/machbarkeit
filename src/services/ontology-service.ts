...
export async function getOntology(module: Module, searchText: string = ''): Promise<[Criterion[] | null, string]> {
	...
	try {
		if (searchText.length > 0) {
			apiResponse = await axios.post(process.env.BACKEND_API_BASE + '/mdr/ontology/concepts/search',
				{
					module_id: module.id,
			...
			apiResponse = await axios.get(process.env.BACKEND_API_BASE + '/mdr/ontology/tree/' + module.id,
...
export async function getConcept(id: string): Promise<Criterion | null> {
	...
	try {
		response = await axios.get(process.env.BACKEND_API_BASE + '/mdr/ontology/concepts/' + id, {
			withCredentials: true,
		})
